...
- `pi_monitor/docs/semantic/{adr,invariants,contracts,constraints}/`
  (pi_monitor's primary ADRs live at `pi_monitor/docs/semantic/adr/`)
...
| Anchor | Lives in | Title (abridged) |
|--------|----------|------------------|
| `@ADR-0019` | `pi_monitor/docs/semantic/adr/0019-live-campaign-sequencing.md` | cross-repository ownership and sequential integration (**superseded** by `@ADR-0001` + `@ADR-0013`) |
| `@ADR-0021` | `pi_monitor/docs/semantic/adr/0021-self-supervision-pi-monitor.md` | self-supervision of pi_monitor |
| `@ADR-0024` | `pi_monitor/docs/semantic/adr/0024-closed-strict-task-marker-vocabulary.md` | closed, strict task-marker vocabulary for the Spec-Kit WorkSource |
| `@ADR-0027` | `pi_monitor/docs/semantic/adr/0027-tick-drift-wedge.md` | tick-drift wedge: nudges worker when raw `tasks.md` checkboxes haven't ticked despite META/attestation/worktree churn; narrower than `no_progress`; routes through existing judge + `RecoveryPolicy` machinery; offers cycle's META + attestation close-out as valid alternative |
